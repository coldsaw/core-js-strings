...
function isPalindrome(str) {
  const newStr = str.replace(/[^a-zA-Z]+/g, '').toUpperCase();
...
function reverseWords(str) {
  const arr = str.split(' ');
  const revertedArr = [];
  arr.forEach((el) => revertedArr.push(el.split('').reverse().join('')));
...
function encodeToRot13(str) {
  const orig = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split('');
  const decoded = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split(
    ''
  );
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (orig.includes(str[i])) {
      result += decoded[orig.findIndex((el) => el === str[i])];
    } else {
      result += str[i];
    }
  }
